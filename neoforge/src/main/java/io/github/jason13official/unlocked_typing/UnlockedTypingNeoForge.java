...
import io.github.jason13official.unlocked_typing.impl.common.registry.ModMenus;
...
public class UnlockedTypingNeoForge {
  ...
  public UnlockedTypingNeoForge(final IEventBus modEventBus) {
    ...
    EVENT_BUS.addListener((Consumer<FMLCommonSetupEvent>) event -> UnlockedTyping.init());

    NeoForge.EVENT_BUS.addListener((Consumer<AddServerReloadListenersEvent>) event -> {
      event.addListener(UnlockedTyping.identifier(Constants.MOD_ID), new ResourceReloadListener());
    });

    if (FMLLoader.getCurrent().getDist() == Dist.CLIENT) {
      new UnlockedTypingClientNeoForge(EVENT_BUS);
    }
  }
  ...
  public static class ResourceReloadListener extends SimplePreparableReloadListener<Void> {
    ...
    public String getName() {
      return UnlockedTyping.identifier(Constants.MOD_ID).toString();
    }
